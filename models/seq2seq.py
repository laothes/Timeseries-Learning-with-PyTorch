...
class LSTMEncoder(nn.Module):
    def __init__(self, input_size, hidden_size, embedding_size, num_layers=1, dropout=0):
        ...
        self.lstm = nn.LSTM(embedding_size, hidden_size, batch_first=True, num_layers=num_layers, dropout=dropout)
...
class LSTMDecoder(nn.Module):
    def __init__(self, output_size, hidden_size, embedding_size, num_layers=1, dropout=0):
        super().__init__()
...
class LSTMEncoderDecoder(nn.Module):
    ...
    def forward(self, x, y, teacher_forcing_ratio = 1):
        ...
        for i in range(target_len):
            ...
            decoder_input = y[:, i, :] if teacher_forcing else output.argmax(dim=2)  # [batch size, 1, output size]
